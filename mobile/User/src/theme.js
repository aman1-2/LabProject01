...
/** Prototype `.cal-grid` / `.cal-day` — doctor + slot calendars. */
export const calendar = StyleSheet.create({
  ...
  dayLabel: { fontSize: 13, fontWeight: '700', color: colors.ink },
  dayMuted: { color: colors.muted2, fontWeight: '600' },
  daySelected: { backgroundColor: colors.blue600 },
  daySelectedLabel: { color: colors.white },
});

/** Prototype `.slot`. */
export const slot = StyleSheet.create({
  base: {
    flex: 1,
    paddingVertical: 11,
    alignItems: 'center',
  ...
  label: { fontWeight: '700', fontSize: 13.5, color: colors.ink },
  selected: { backgroundColor: colors.blue600, borderColor: colors.blue600 },
  selectedLabel: { color: colors.white },
  disabled: { backgroundColor: '#F5F6F8', borderColor: colors.border },
  disabledLabel: { color: colors.muted2 },
});

/** Prototype active-booking card: blue600 -> #1B2C6B gradient. */
export const activeBookingGradient = [colors.blue600, colors.blueGradientEnd];

/** Prototype `.sheet` — bottom sheet. */
export const sheet = StyleSheet.create({
  overlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(16,17,20,0.45)' },
  body: {
    position: 'absolute',
    left: 0,
    right: 0,
  ...
  grabber: { width: 40, height: 4, borderRadius: 99, backgroundColor: colors.border, alignSelf: 'center', marginBottom: 18 },
});

/** DESIGN_SPEC §2.5 empty state: 1.5px DASHED border, radius 20, padding 44/24. */
export const emptyState = StyleSheet.create({
  ...
  title: { fontWeight: '800', fontSize: 15, color: colors.ink, textAlign: 'center' },
  body: { color: colors.muted, fontSize: 13.5, textAlign: 'center', marginTop: 6, maxWidth: 360, lineHeight: 20 },
});
