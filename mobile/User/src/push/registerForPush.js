...
export async function registerForPushNotifications() {
  try {
    if (isExpoGo) {
      // Reported rather than thrown: the caller carries on and the Jobs list
      // keeps polling, so the app is fully usable without push.
      return { registered: false, reason: 'Push is unavailable in Expo Go — use a development build.' };
    }
    ...
    // Loaded here rather than imported at the top of the file — see
    // loadNotifications above for why that distinction matters.
